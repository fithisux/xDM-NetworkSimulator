 ...
 */
public final class lightWeightGraph
{
    public lightWeightNode[] nodes;
    public List<lightWeightEdge> cache;
    public lightWeightGraph(int nodesnum)
    {        
        cache=new ArrayList<lightWeightEdge>();
        nodes=new lightWeightNode[nodesnum];
        for(int i=0;i<nodes.length;i++)
        {
            nodes[i]=new lightWeightNode(i);
        }
    }
    ...
    public void remove(lightWeightEdge e)
    {
        this.nodes[e.from].remove(e);
        cache.remove(e);
    }
    
    public void add(lightWeightEdge e)
    {
        this.nodes[e.from].add(e);
        cache.add(e);
    }
}
