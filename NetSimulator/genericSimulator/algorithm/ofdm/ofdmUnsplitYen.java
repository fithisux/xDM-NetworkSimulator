...
import genericSimulator.algorithm.cspfElement;
 ...
 * @author vanag
 */
public class ofdmUnsplitYen extends ofdmUnsplitCSPF
{
    public ofdmUnsplitYen(genericRCM rcm)
    {
        super(rcm);
        genericCSPF[] shpa;
        shpa=new genericYenAlgorithm[this.computations.cspfgraphs.length];
        
        int k=-1;
        String kvalue=this.algo_params.get("kvalue");
        if(kvalue==null){
            System.out.println("No kvalue for yen");
            System.exit(0);
        } else {
        ...
        {
            shpa[i]=new genericYenAlgorithm(this.computations.cspfgraphs[i],k);
        }
        this.computations.shpa=shpa;        
    }
}
