 ...
 * @author  vanag
 */
public final class distroDeterministic extends genericDistribution
{
    int quantity;
    int active_quantity;
    ...
    public void initialize(distributionPrefs prefs,Random go)
    {
        ...
        if(this.params.containsKey("quantity"))
        {
            active_quantity=quantity=Integer.parseInt(this.params.get("quantity"));
        }
        else
        {
            System.out.println("Deterministic without quantity");
            System.exit(0);
        }
    }
    ...
    {               
        active_quantity= (int) Math.round(quantity/(1+epoch*this.xi));
    }
    
    
    public String toString()
    {
        String s="A deterministic with val "+(quantity);
        return s;
    }
